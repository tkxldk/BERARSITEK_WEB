        ...
        <div className="grid grid-cols-1 gap-[5px] md:grid-cols-2 md:gap-[10px] lg:grid-cols-5 lg:gap-[20px] xl:grid-cols-5 xl:gap-[30px]">
          {categoriesData &&
            categoriesData.map((i) => {
              ...
              return (
                <div className="w-full h-[100px] flex items-center justify-start cursor-pointer overflow-hidden" key={i.id} onClick={() => handleSubmit(i)}>
                  <div>{i.icon}</div>
                  <div>
                    <h5 className={`ml-2 text-[18px] uppercase leading-[1.3] font-semibold font-[Poppins] `}>{i.title}</h5>
                  </div>
