    ...
    <div className={`py-4 min-h-screen ${styles.section}`}>
      <div className="w-full flex items-center justify-between">
        <div className="flex items-center">
          <RiShoppingBag3Line size={30} color="#0B3F9C" />
        ...
        data?.cart.map((item, index) => {
          return (
            <div className="w-full flex items-start mb-5">
              ...
              {!item.isReviewed && data?.status === "Completed" ? (
                <div className={`${styles.button} text-[#fff] font-semibold`} style={{ backgroundColor: "#0B3F9C", width: 175 }} onClick={() => setOpen(true) || setSelectedItem(item)}>
                  Write a review
                </div>
              ) : null}
            </div>
      ...
      {open && (
        <div className="w-full fixed top-0 left-0 h-screen bg-[#0005] z-50 flex items-center justify-center">
          <div className="w-[50%] h-min bg-[#fff] shadow rounded-md p-3">
            ...
            <div className={`${styles.button} text-white text-[20px] ml-3`} style={{ backgroundColor: "#0B3F9C" }} onClick={rating > 1 ? reviewHandler : null}>
              Submit
            </div>
          </div>
        </div>
      ...
      <div className="w-full 800px:flex items-center">
        <div className="w-full 800px:w-[60%]">
          <h4 className="pt-3 text-[20px] font-[600]"> Billing Info :</h4>
        ...
        <div className="w-full 800px:w-[40%]">
          ...
          <h4 className="pt-3 text-[20px]">Status : {data?.paymentInfo?.status ? data?.paymentInfo?.status : "Not Paid"}</h4>
        </div>
      </div>
      <br />
      <Link to="/">
        <div className={`${styles.button} text-white font-semibold`} style={{ backgroundColor: "#0B3F9C", width: 150 }}>
          Send Message
        </div>
      </Link>
      <br />
