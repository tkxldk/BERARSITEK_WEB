            ...
            <div className="block w-full 800px:flex">
                ...
                <div className="w-full flex">
                  {data &&
                    data.images.map((i, index) => (
                      <div className={`${select === 0 ? "null" : "null"} cursor-pointer`}>
                        <img src={`${backend_url}${i}`} alt="" className="h-[168px] overflow-hidden mr-3 mt-3" onClick={() => setSelect(index)} />
                      </div>
                    ))}
                  <div className={`${select === 1 ? "null" : "null"} cursor-pointer`}></div>
                </div>
              </div>
              <div className="w-full 800px:w-[50%] pt-5">
                ...
                <div className="flex pt-3">
                  <h4 className={`${styles.productDiscountPrice}`}>Rp. {data.discountPrice}</h4>
                  <h3 className={`${styles.price}`}>{data.originalPrice ? data.originalPrice + "Rp. " : null}</h3>
                </div>

                <div className="flex items-center mt-12 justify-between pr-3">
                  <div>
                    <button className="bg-[#0B3F9C] text-white font-bold px-4 py-2 shadow-lg hover:opacity-75 transition duration-300 ease-in-out mr-1" onClick={decrementCount}>
                      -
                    </button>
                    <span className="bg-gray-200 text-gray-800 font-bold px-4 py-2">{count}</span>
                    <button className="bg-[#0B3F9C] text-white font-bold px-4 py-2 shadow-lg hover:opacity-75 transition duration-300 ease-in-out ml-1" onClick={incrementCount}>
                      +
                    </button>
                  </div>
                  <div>
                ...
                <div className={`${styles.button} !mt-6 !rounded !h-11 flex items-center`} style={{ backgroundColor: "#0B3F9C" }} onClick={() => addToCartHandler(data._id)}>
                  <span className="text-white flex items-center font-semibold">
                    Buy Blueprint <IoReceiptOutline className="ml-1" />
                  </span>
                </div>
                <div className="flex items-center pt-8">
                  ...
                  <div className="pr-8">
                    ...
                    <h5 className="pb-3 text-[15px] font-semibold">{averageRating} / 5 Ratings</h5>
                  </div>
                  <div className={`${styles.button} bg-[#6443d1] mt-4 !rounded !h-11`} style={{ backgroundColor: "#0B3F9C" }} onClick={handleMessageSubmit}>
                    <span className="text-white flex items-center font-semibold">
    ...
    <div className="bg-[#f5f6fb] px-3 800px:px-10 py-2 rounded">
      ...
      {active === 2 ? (
        <div className="w-full min-h-[40vh] flex flex-col items-center py-3 overflow-y-scroll">
          ...
          <div className="w-full flex justify-center">{data && data.reviews.length === 0 && <h5>No Reviews have for this blueprint!</h5>}</div>
        </div>
      ) : null}
        ...
        <div className="w-full block 800px:flex p-5">
          <div className="w-full 800px:w-[50%]">
            <Link to={`/shop/preview/${data.shop._id}`}>
              <div className="flex items-center">
                <img src={`${backend_url}${data?.shop?.avatar}`} className="w-[50px] h-[50px] rounded-full" alt="" />
                <div className="pl-3">
                  <h3 className={`${styles.shop_name}`}>{data.shop.name}</h3>
                  <h5 className="pb-2 text-[15px] font-semibold">{averageRating} / 5 Ratings</h5>
                </div>
              </div>
            </Link>
          ...
          <div className="w-full 800px:w-[50%] mt-5 800px:mt-0 800px:flex flex-col items-end">
            <div className="text-left">
              ...
              <Link to="/">
                <div className={`${styles.button} !rounded-[4px] !h-[39.5px] mt-3 bg-[#0B3F9C] font-semibold`}>
