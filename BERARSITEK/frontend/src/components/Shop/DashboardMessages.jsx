...
const SellerInbox = ({ scrollRef, setOpen, newMessage, setNewMessage, sendMessageHandler, messages, sellerId, userData, activeStatus, handleImageUpload }) => {
  return (
    <div className="w-full min-h-full flex flex-col justify-between">
      {/* message header */}
      <div className="w-full flex p-3 items-center justify-left bg-[#d8d8d8]">
        <AiOutlineArrowLeft size={20} className="cursor-pointer" onClick={() => setOpen(false)} />
        <div className="flex">
          ...
          messages.map((item, index) => {
            return (
              <div className={`flex w-full my-2 ${item.sender === sellerId ? "justify-end" : "justify-start"}`} ref={scrollRef}>
                ...
                {item.images && <img src={`${backend_url}${item.images}`} className="w-[300px] h-[300px] object-cover rounded-[10px] mr-2" />}
                {item.text !== "" && (
                  <div>
      ...
      <form aria-required={true} className="p-3 relative w-full flex justify-between items-center" onSubmit={sendMessageHandler}>
        <div className="w-[30px]">
          ...
          <label htmlFor="image">
            <HiOutlinePhotograph className="cursor-pointer" size={20} />
          </label>
        </div>
        <div className="w-full">
          ...
          <label htmlFor="send">
            <TbSend size={20} className="absolute right-6 top-5 cursor-pointer" />
          </label>
        </div>
      </form>
    </div>
