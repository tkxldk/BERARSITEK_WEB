    ...
    <div className={`py-4 min-h-screen ${styles.section}`}>
      <div className="w-full flex items-center justify-between">
        ...
        <Link to="/dashboard-orders">
          <div className={`${styles.button} !bg-[#0B3F9C] !rounded-[4px] text-[#fff] font-[500] !h-[45px] text-[18px]`}>Order List</div>
        </Link>
      </div>
      ...
      {data?.status !== "Processing refund" && data?.status !== "Refund Success" && (
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-[200px] mt-2 border h-[35px] rounded-[5px]">
          {["Processing", "Completed"].slice(["Processing", "Completed"].indexOf(data?.status)).map((option, index) => (
      ...
      <div className={`${styles.button} mt-5 !bg-[#0B3F9C] !rounded-[4px] text-[#fff] font-[600] !h-[45px] text-[18px]`} onClick={data?.status !== "Processing refund" ? orderUpdateHandler : refundOrderUpdateHandler}>
        Update Status
      </div>
    </div>
  );
