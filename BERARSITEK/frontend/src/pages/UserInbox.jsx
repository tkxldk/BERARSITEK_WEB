  ...
  return (
    <div className="w-full">
      {!open && (
        <>
          <Header />
          <h1 className="text-center text-[30px] py-3 font-Poppins">Chat</h1>
          {/* All messages list */}
          {conversations &&
...
const MessageList = ({ data, index, setOpen, setCurrentChat, me, setUserData, userData, online, setActiveStatus }) => {
  ...
  return (
    <div className="flex items-center justify-center">
      <div
        className={`w-[90%] flex p-3 px-3 ${active === index ? "bg-[#00000010]" : "bg-transparent"}  cursor-pointer`}
      ...
      >
        ...
        <div className="pl-3">
          ...
          <p className="text-[16px] text-[#000c]">
            {data?.lastMessageId !== userData?._id ? "You:" : userData?.name.split(" ")[0] + ": "} {data?.lastMessage}
          </p>
        </div>
      </div>
    </div>
...
const SellerInbox = ({ setOpen, newMessage, setNewMessage, sendMessageHandler, messages, sellerId, userData, activeStatus, scrollRef, handleImageUpload }) => {
  return (
    <div className="w-[full] min-h-full flex flex-col justify-between p-5">
      {/* message header */}
      <div className="w-full flex p-3 items-center justify-left bg-[#fff]">
        <AiOutlineArrowLeft size={25} className="cursor-pointer" onClick={() => setOpen(false)} style={{ color: "#0B3F9C" }} />
        <div className="flex">
          <img src={`${backend_url}${userData?.avatar}`} alt="" className="ml-10 w-[60px] h-[60px] rounded-full" />
          <div className="pl-3">
        ...
        {messages &&
          messages.map((item, index) => (
            <div className={`flex w-full my-2 ${item.sender === sellerId ? "justify-end" : "justify-start"}`} ref={scrollRef}>
              {item.sender !== sellerId && <img src={`${backend_url}${userData?.avatar}`} className="w-[40px] h-[40px] rounded-full mr-3" alt="" />}
              {item.images && <img src={`${backend_url}${item.images}`} className="w-[300px] h-[300px] object-cover rounded-[10px] ml-2 mb-2" />}
              {item.text !== "" && (
                <div>
                  <div className={`w-max p-2 rounded ${item.sender === sellerId ? "bg-[#D8D8D8]" : "bg-[#C8E6F1]"} text-[#000] h-min`}>
                    <p>{item.text}</p>
                  </div>
      ...
      <form aria-required={true} className="p-3 relative w-full flex justify-between items-center" onSubmit={sendMessageHandler}>
        <div className="w-[30px]">
          ...
          <label htmlFor="image">
            <HiOutlinePhotograph className="cursor-pointer" size={25} style={{ color: "#0B3F9C" }} />
          </label>
        </div>
        <div className="w-full">
          ...
          <label htmlFor="send">
            <TbSend size={20} className="absolute right-6 top-5 cursor-pointer" style={{ color: "#0B3F9C" }} />
          </label>
        </div>
      </form>
    </div>
